...
CREATE TABLE `scs_consult` (
  ...
  `consult_start` time COMMENT '諮商開始時間',
  `consult_end` time COMMENT '諮商結束時間',
...
CREATE TABLE `scs_general` (
  `stu_id` mediumint(9) unsigned NOT NULL COMMENT '學生編號',
  `school_year` tinyint(3) unsigned NOT NULL COMMENT '學年度',
  `stu_grade` tinyint(3) unsigned NOT NULL default '0' COMMENT '年級',
...
CREATE TABLE `scs_guardian` (
  ...
  `guardian_sex` enum('','男','女') COMMENT '性別',
...
CREATE TABLE `scs_students` (
  ...
  `stu_name` varchar(100) NOT NULL default '' COMMENT '姓名',
  ...
  `stu_blood` varchar(20) default '' COMMENT '血型',
